...
public class LocalEventStore implements Datalake.EventStore {
	public static final String EventExtension = ZimExtension;
	public static final String SessionExtension = ".event.blob";
	private File root;

	public LocalEventStore(File root) {
		this.root = root;
	}

	@Override
	public Stream<Tank> tanks() {
		return FS.foldersIn(root).map(LocalEventTank::new);
	}

	@Override
	public Tank tank(String name) {
		return new LocalEventTank(new File(root, name));
	}
