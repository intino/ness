...
public class SetSessionManager_ {
	...
	public void should_define_variable() {
		...
		FileDatalake datalake = new FileDatalake(new File("temp"));
		Digester digester = new Digester(datalake, new File("test-res/setstage"));
		digester.push(sessions());
		digester.seal();
		assertTrue(new File("temp/sets/tank1/201809/" + FileSetStore.MetadataFilename).exists());
		Assert.assertEquals(1, datalake.setStore().tank("tank1").on(timetag).set("set1").variables().count());
		Assert.assertEquals("10", datalake.setStore().tank("tank1").on(timetag).set("set1").variable("var").value);
