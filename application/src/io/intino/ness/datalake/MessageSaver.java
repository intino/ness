...
class MessageSaver {
	...
	static void save(File inlFile, List<Message> messages, StandardOpenOption... options) {
		StringBuilder builder = new StringBuilder();
		for (Message m : messages) builder.append(m.toString()).append("\n\n");
		write(inlFile, builder.toString());
	}

	private static synchronized void write(File inlFile, String textMessage, StandardOpenOption... options) {
		try {
