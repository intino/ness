...
public class NessieSlackBot extends Bot {
	...
	public NessieSlackBot(NessBox box) {
		...
		add("append-function".toLowerCase(), java.util.Arrays.asList("name", "code"), java.util.Arrays.asList(), "Create a function associated to an input topic and output topic", (properties, args) -> NessieSlackBotActions.addFunction(box, properties, args.length > 0 ? args[0] : "", args.length > 1 ? args[1] : ""));
		...
		add("plug".toLowerCase(), java.util.Arrays.asList("functionName", "input", "output"), java.util.Arrays.asList(), "Connect a source and destination topics by a `function`", (properties, args) -> NessieSlackBotActions.pump(box, properties, args.length > 0 ? args[0] : "", args.length > 1 ? args[1] : "", args.length > 2 ? args[2] : ""));
