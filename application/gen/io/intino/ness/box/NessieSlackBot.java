...
public class NessieSlackBot extends Bot {
	...
	public NessieSlackBot(NessBox box) {
		...
		add("tank".toLowerCase(), "", java.util.Arrays.asList("name"), java.util.Arrays.asList("tag", "rename", "seal", "migrate"), "Select a tank to operate with", (properties, args) -> nessieSlack.tank(properties, args.length > 0 ? args[0] : ""));
		...
		add("add-pipe".toLowerCase(), "", java.util.Arrays.asList("from", "to", "functionName"), java.util.Arrays.asList(), "Connects source and destination topics real-time, optionaly converting the message with a function", (properties, args) -> nessieSlack.addPipe(properties, args.length > 0 ? args[0] : "", args.length > 1 ? args[1] : "", args.length > 2 ? args[2] : ""));
