...
public class NessieSlackBot extends Bot {
	...
	public NessieSlackBot(NessBox box) {
		...
		add("start-aqueduct".toLowerCase(), "", java.util.Arrays.asList("name", "originURL", "user", "password", "originTopic", "destinationTank", "functionName"), java.util.Arrays.asList(), "connect out jms topic with a ness tank", (properties, args) -> nessieSlack.startAqueduct(properties, args.length > 0 ? args[0] : "", args.length > 1 ? args[1] : "", args.length > 2 ? args[2] : "", args.length > 3 ? args[3] : "", args.length > 4 ? args[4] : "", args.length > 5 ? args[5] : "", args.length > 6 ? args[6] : ""));
