...
public class Tasks {
	...
	public static void init(KonosTasker tasker, NessBox box) {
		JobDetail job;
		try {
			job = newJob(SortTanksTask.class).withIdentity("sortTanks").build();
			job.getJobDataMap().put("box", box);
			tasker.scheduleJob(job, newSet(newTrigger().withIdentity("Application#sortTanks").withSchedule(cronSchedule("0 0 4 1/1 * ? *")).build()), true);
