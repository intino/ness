...
public class EventSession implements io.intino.ness.core.EventSession {
	...
	private BufferedWriter createWriter(Fingerprint fingerprint) {
		return new BufferedWriter(new OutputStreamWriter(zipStream(provider.outputStream(fingerprint.name(), event))));
	}
