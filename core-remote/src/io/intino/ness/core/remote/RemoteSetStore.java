...
public class RemoteSetStore implements Datalake.SetStore {
	...
	public Stream<Tank> tanks() {
		try {
			return Arrays.stream(fs.listStatus(root)).
					...
					map(s -> new RemoteSetTank(s.getPath()));
		} catch (IOException e) {
			Logger.error(e);
			return null;
		}
	...
	public void put(ZetStream stream, String name) {
		try {
			new SequenceSetWriter(fs, name).write(stream);
		} catch (IOException e) {
			Logger.error(e);
		}
	}
