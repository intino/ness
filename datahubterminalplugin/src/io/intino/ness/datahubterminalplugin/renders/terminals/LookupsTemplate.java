...
public class LookupsTemplate extends Template {
	...
	public RuleSet ruleSet() {
		return new RuleSet().add(
			...
			rule().condition((trigger("namespace"))).output(literal("private java.sql.Connection ")).output(mark("", "validName", "firstLowerCase")).output(literal(";")),
			...
			rule().condition((type("lookup")), (trigger("getter"))).output(literal("public ")).output(mark("qn")).output(literal(" ")).output(mark("name", "firstLowerCase")).output(literal("() {\n\tif (this.")).output(mark("namespace", "validName", "firstLowerCase")).output(literal(" == null) {\n\t\ttry {\n\t\t\tthis.")).output(mark("namespace", "validName", "firstLowerCase")).output(literal(" = java.sql.DriverManager.getConnection(\"jdbc:sqlite:\" + new java.io.File(lookupsDirectory,  \"")).output(mark("namespace", "firstLowerCase")).output(literal(".db\"));\n    \t\tthis.")).output(mark("namespace", "validName", "firstLowerCase")).output(literal(".setAutoCommit(false);\n\t\t} catch (java.sql.SQLException e) {\n\t\t\tio.intino.alexandria.logger.Logger.error(e);\n\t\t}\n\t}\n\tif (this.")).output(mark("name", "firstLowerCase")).output(literal(" == null) this.")).output(mark("name", "firstLowerCase")).output(literal(" = new ")).output(mark("qn")).output(literal("(")).output(mark("namespace", "validName", "firstLowerCase")).output(literal(");\n\treturn this.")).output(mark("name", "firstLowerCase")).output(literal(";\n}"))
		);
	}
}