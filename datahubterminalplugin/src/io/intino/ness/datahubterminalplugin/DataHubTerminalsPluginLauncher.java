...
public class DataHubTerminalsPluginLauncher extends PluginLauncher {
	...
	private void publishOntology(NessGraph graph, Map<String, String> versions, File tempDir) {
		try {
			...
			published.set(new OntologyPublisher(new File(tempDir, "ontology"), graph, configuration(), moduleStructure().resDirectories,versions, systemProperties(), invokedPhase, logger()).publish() & published.get());
