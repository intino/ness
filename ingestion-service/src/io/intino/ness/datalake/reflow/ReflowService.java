...
public class ReflowService implements Consumer {
	...
	private List<io.intino.ness.graph.Datalake.Tank> pausedTanks;

	public ReflowService(NessServiceBox box) {
		this.box = box;
	}
...
//		if(box.busService().isPersistent()) box.restartBus(true); TODO falla al arrancar con persistencia
	...
	private void pauseTanks() {
		this.pausedTanks = box.graph().datalake().tankList().stream().filter(io.intino.ness.graph.Datalake.Tank::active).collect(Collectors.toList());
