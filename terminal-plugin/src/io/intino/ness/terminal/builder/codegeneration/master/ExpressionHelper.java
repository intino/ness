...
public class ExpressionHelper {
	...
	private static String type(Node node) {
		...
		if (list)
			return EntityFrameCreator.ListTypes.getOrDefault(aspect, "List<" + firstUpperCase().format(node.name()).toString() + ">");

		boolean set = node.conceptList().stream().anyMatch(a -> a.name().equals("Set"));
		if (set)
			return EntityFrameCreator.SetTypes.getOrDefault(aspect, "Set<" + firstUpperCase().format(node.name()).toString() + ">");

		return EntityFrameCreator.TheTypes.getOrDefault(aspect, firstUpperCase().format(node.name()).toString());
	}
