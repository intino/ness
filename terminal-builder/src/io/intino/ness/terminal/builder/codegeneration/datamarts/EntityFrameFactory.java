...
public class EntityFrameFactory implements ConceptRenderer {
	...
	private String baseEntityName() {
		return workingPackage + "." + Formatters.firstUpperCase(datamart.name$()) + "Entity";
	}
	...
	private String calculateEntityPath(Entity entity, String thePackage) {
		return thePackage + DOT + "entities" + DOT
				...
				+ firstUpperCase().format(Formatters.javaValidName().format(entity.core$().name()).toString());
	}

	private String calculateDecorableEntityPath(Node node, String aPackage) {
		return aPackage + DOT + "entities" + DOT + firstUpperCase().format(Formatters.javaValidName().format(node.name()).toString());
	}
